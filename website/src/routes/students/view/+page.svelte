		...
		<div class="bg-bg-light p-5 rounded-lg shadow-lg text-white m-5 z-40 text-lg">
			...
			<div class="flex justify-between gap-4 mt-4">
				...
				<button class="bg-warn-red hover:bg-red-700 text-white font-bold py-2 px-4 rounded"
						on:click={deleteStudent}>Löschen
				</button>
			</div>
		</div>
	</div>
	...
	{#if student !== null}
		<div class="text-white flex-row pb-2 text-lg">
			...
			<div class="mb-2 fill-white flex w-full justify-center">
				...
				<a href="/students/edit?id={studentId}" class="scale-80 fill-white pt-1 pl-2">
					<EditIcon />
				</a>
			</div>
			<p class="mt-2 font-bold">Sprints</p>
			<div class="flex-row flex font-bold">
				<p><span class="text-tx-gray font-normal">Ø-Note:</span> {student.sprint.avg_grade.replace('.', ',')}
				</p>
				<p class="absolute left-[50%]"><span
					class="text-tx-gray font-normal">Ø-Zeit:</span> {formatTime(student.sprint.avg_time, 0)}</p>
			</div>
			<p class="mt-2 font-bold">Rundenlauf</p>
			<div class="flex-row flex font-bold">
				<p><span class="text-tx-gray font-normal">Ø-Note:</span> {student.lap_run.avg_grade.replace('.', ',')}
				</p>
				<p class="absolute left-[50%]"><span
					class="text-tx-gray font-normal">Ø-Zeit:</span> {formatTime(student.lap_run.avg_time, 1)}</p>
			</div>
			<p class="mt-2 font-bold">Gesamt</p>
			<p class="font-bold"><span
				class="text-tx-gray font-normal">Ø-Note:</span> {student.global_avg_grade.replace('.', ',')}</p>
		</div>
