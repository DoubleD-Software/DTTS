...
<div class="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center p-5 text-xl z-40">
	<div class="bg-bg-light p-4 rounded-lg w-full text-white">
		...
		<div class="flex justify-center">
			{#if type === 0}
				<input id="first" class="w-14 rounded bg-bg-lightest text-center" type="number"
					   on:input={validateFirstInput} bind:value={first}
					   placeholder="sek"
					   on:keydown={(e) => e.key === 'Enter' && document.getElementById('second').focus()}>
				<span>,</span>
				<input id="second" class="w-14 rounded bg-bg-lightest text-center mr-2" type="number"
					   on:input={validateSecondInput} bind:value={second}
